...
PLOTLY_TEMPLATE = dict(
    ...
    colorway=[CYAN, GOLD, TEAL, ROSE, "#9B59B6", "#2ECC71"],
)
...
.accuracy-badge {{
    ...
    background: linear-gradient(135deg, rgba(0,212,255,0.15), rgba(0,229,195,0.1));
...
# ──────────────────────────────────────────────
# DATOS DEL ANÁLISIS (extraídos del notebook)
# ──────────────────────────────────────────────
np.random.seed(42)
...
# --- PCA sintético (distribución representativa) ---
def gen_cluster_points(n, cx, cy, spread_x=0.6, spread_y=0.5):
    return pd.DataFrame({
        "PC1": np.random.randn(n) * spread_x + cx,
        "PC2": np.random.randn(n) * spread_y + cy,
    })
...
# Matriz de confusión (representativa, 91.1% accuracy)
cm = np.array([
    [2687,  298,    4],
    [ 136,  843,   88],
    [   2,    5,  399],
...
modelos_df = pd.DataFrame({
    "Modelo": ["Ridge (α=500)", "OLS completo", "OLS reducido\n(3 vars)"],
...
coefs_df = pd.DataFrame({
    "Variable": ["Nivel educativo", "Edad", "Jerarquía laboral",
                 "Sector formal", "Horas semanales", "Jefatura de hogar"],
    "Efecto (% ingreso)": [15.6, 11.3, 3.0, 8.2, 2.1, 4.7],
}).sort_values("Efecto (% ingreso)")

# Scatter residuos sintéticos
n_res = 500
...
with st.sidebar:
    ...
    <div style="font-size:0.78rem;color:{SLATE};line-height:1.8;">
        <b style="color:{WHITE};">Muestra limpia</b><br>
        37.312 individuos<br>
        113 variables<br><br>
        <b style="color:{WHITE};">Fuente</b><br>
        INDEC — EPH<br>
        3er trimestre 2025
    </div>
    """, unsafe_allow_html=True)
    ...
    <div style="font-size:0.75rem;color:{SLATE};">
        <b style="color:{WHITE};">David Palacio Velásquez</b><br>
...
if "Resumen" in page:
    ...
    <div class="hero">
        ...
        <h1 class="hero-title">
            Mercado Laboral<br>
            <span>Argentino</span> — EPH T3-2025
        </h1>
    ...
    metrics = [
        ...
        ("91,1 %", "Accuracy de clasificación", "KNN · K = 17"),
    ...
    # ── Distribución de clusters + Distribución sectorial ──
    c1, c2 = st.columns([1.15, 1])

    with c1:
        st.markdown(f"<div class='section-title'>Cuatro perfiles del mercado laboral</div>",
                    unsafe_allow_html=True)
        st.markdown(f"""<div class='section-subtitle'>
            K-Means con K=4 sobre componentes PCA. La condición de actividad
            es la dimensión de variación más relevante en los datos.
        ...
        fig_bar.update_layout(
            **PLOTLY_TEMPLATE,
            ...
            yaxis=dict(range=[0, 60], **PLOTLY_TEMPLATE["yaxis"]),
            bargap=0.3,
        )
        st.plotly_chart(fig_bar, use_container_width=True, config={"displayModeBar": False})

    with c2:
        st.markdown(f"<div class='section-title'>Composición sectorial</div>",
                    unsafe_allow_html=True)
        st.markdown(f"""<div class='section-subtitle'>
            Distribución de ocupados por categoría de empleo.
            El sector informal representa más de un cuarto del total.
        </div>""", unsafe_allow_html=True)
        ...
        fig_pie.add_annotation(
            text="Ocupados", x=0.5, y=0.55, showarrow=False,
            font=dict(size=11, color=SLATE, family="DM Sans"),
        )
        fig_pie.add_annotation(
            text="T3-2025", x=0.5, y=0.42, showarrow=False,
            font=dict(size=11, color=SLATE, family="DM Sans"),
        )
    ...
    st.markdown(f"<div class='section-title'>Ingreso promedio por perfil laboral</div>",
                unsafe_allow_html=True)
    ...
    fig_ing.update_layout(
        **PLOTLY_TEMPLATE,
        ...
        height=300,
        margin=dict(l=0, r=0, t=20, b=0),
        yaxis_title="Ingreso promedio (ARS)",
        yaxis=dict(range=[0, 1_100_000], tickformat="$,.0f",
                   **PLOTLY_TEMPLATE["yaxis"]),
        bargap=0.35,
    )
    st.plotly_chart(fig_ing, use_container_width=True, config={"displayModeBar": False})

    # ── Hallazgos clave ──
    st.markdown(f"<div class='section-title'>Hallazgos principales</div>",
                unsafe_allow_html=True)
    ...
    hallazgos = [
        (CYAN,  "🏗️ Estructura del mercado",
         ...
         "El clustering captura la separación entre ocupados plenos y no ocupados "
         "como la más relevante en los datos."),
        (GOLD,  "🎓 Educación e ingreso",
         "Cada escalón educativo adicional se asocia con <strong>+15,6 % de ingreso</strong>, "
         "resultado consistente con décadas de literatura en economía laboral "
         "(Becker, 1964; Mincer, 1974)."),
        (TEAL,  "📍 Frontera formal-informal",
         "Los errores del clasificador KNN se concentran en la frontera "
         "<strong>formal-informal</strong>, ya que ambos sectores comparten muchas "
         "características observables en la EPH."),
    ]
    for col, (color, titulo, texto) in zip([h1, h2, h3], hallazgos):
        with col:
            ...
            <div class="metric-card" style="text-align:left;padding:20px;">
                ...
                <div style="font-size:0.88rem;color:{SLATE};line-height:1.7;">
                    {texto}
                </div>
            </div>
            """, unsafe_allow_html=True)
...
# ══════════════════════════════════════════════
    ...
    <div class="hero">
        ...
        <p class="hero-subtitle">
            ...
            (21,4 % de varianza explicada). La selección de K se justifica por el método
            del codo: la reducción marginal de inercia cae −58 % al pasar de K=4 a K=5.
        </p>
    </div>
    """, unsafe_allow_html=True)

    # ── PCA scatter ──
    st.markdown(f"<div class='section-title'>Espacio PCA — Distribución de clusters</div>",
                unsafe_allow_html=True)
    ...
    for _, row in clusters_df.iterrows():
        ...
        fig_pca.add_trace(go.Scatter(
            ...
            name=row["Perfil_short"],
            marker=dict(
                color=row["color"], size=5,
                opacity=0.55,
                line=dict(color=row["color"], width=0),
            ),
            hovertemplate=f"<b>{row['Perfil_short']}</b><br>PC1: %{{x:.2f}}<br>PC2: %{{y:.2f}}<extra></extra>",
        ))
    ...
    # ── Tabla de clusters + radar ──
    tc, rc = st.columns([1.1, 1])

    with tc:
        st.markdown(f"<div class='section-title'>Caracterización de los 4 perfiles</div>",
                    unsafe_allow_html=True)
        st.markdown(f"""
                ...
                <tr>
                    <td>
                        <span style="display:inline-block;width:10px;height:10px;
                                     border-radius:50%;background:{row['color']};
                                     margin-right:8px;"></span>
                        {row['Perfil_short']}
                    </td>
                    <td style="color:{SLATE};">{row['n']:,}</td>
                    <td>
                        <span style="color:{row['color']};font-weight:600;">
                            {row['pct']} %
                        </span>
                    </td>
        ...
        <div class="highlight-box" style="margin-top:20px;">
            <strong>Nota metodológica:</strong> La normalización con StandardScaler se aplica
            antes del PCA. DBSCAN fue descartado por la maldición de la dimensionalidad
            con n = 37.312 y p = 116 variables.
        </div>
        """, unsafe_allow_html=True)

    with rc:
        st.markdown(f"<div class='section-title'>Radar de perfiles</div>",
                    unsafe_allow_html=True)

        categorias = ["Ingreso\n(escalado)", "Horas\ntrabajadas", "Tasa de\nactividad",
                      "Nivel\neducativo", "Antigüedad\nestimada"]
        # Valores representativos normalizados 0-1
        valores = [
            [0.22, 0.00, 0.00, 0.45, 0.30],   # Cluster 0
            [0.90, 1.00, 1.00, 0.60, 0.65],   # Cluster 1
            [0.44, 0.73, 0.80, 0.50, 0.40],   # Cluster 2
            [0.08, 0.00, 0.00, 0.35, 0.05],   # Cluster 3
        ]
        ...
        for i, (_, row) in enumerate(clusters_df.iterrows()):
            ...
            fig_radar.add_trace(go.Scatterpolar(
                ...
                fillcolor=row["color"].replace(")", ",0.1)").replace("rgb", "rgba")
                           if "rgb" in row["color"] else row["color"] + "1A",
                opacity=0.85,
            ))
            ...
            **PLOTLY_TEMPLATE,
            polar=dict(
                ...
                radialaxis=dict(visible=True, range=[0, 1],
                                gridcolor=BORDER, tickfont=dict(size=9, color=SLATE)),
...
# ══════════════════════════════════════════════
    ...
    for col, (val, lbl, dlt) in zip([m1, m2, m3, m4], class_metrics):
        with col:
            st.markdown(f"""
            <div class="metric-card">
    ...
    with col_cm:
        st.markdown(f"<div class='section-title'>Matriz de confusión</div>",
                    unsafe_allow_html=True)
        st.markdown(f"""<div class='section-subtitle'>
            Conjunto de test — 4.462 individuos (20 % estratificado)
        </div>""", unsafe_allow_html=True)
        ...
        text_matrix = [[f"{cm_norm[i][j]:.1%}<br><span style='font-size:10px;opacity:0.6;'>"
                        f"n={cm[i][j]}</span>"
        ...
        fig_cm.update_layout(
            **PLOTLY_TEMPLATE,
            ...
            yaxis_title="Real",
            yaxis=dict(autorange="reversed", **PLOTLY_TEMPLATE["yaxis"]),
        )
        st.plotly_chart(fig_cm, use_container_width=True, config={"displayModeBar": False})

    with col_bar:
        st.markdown(f"<div class='section-title'>Métricas por clase</div>",
                    unsafe_allow_html=True)
        st.markdown(f"""<div class='section-subtitle'>
            Precisión, recall y F1-score para cada categoría de empleo
        </div>""", unsafe_allow_html=True)

        metricas_clase = pd.DataFrame({
            "Clase": ["Formal", "Informal", "Serv. doméstico"] * 3,
            "Métrica": (["Precisión"] * 3 + ["Recall"] * 3 + ["F1"] * 3),
            "Valor": [0.95, 0.73, 1.00,   # Precisión
                      0.90, 0.78, 0.98,   # Recall
                      0.93, 0.76, 0.99],  # F1
        })

        fig_met = go.Figure()
        met_colors = [CYAN, TEAL, GOLD]
        for i, met in enumerate(["Precisión", "Recall", "F1"]):
            sub = metricas_clase[metricas_clase["Métrica"] == met]
            fig_met.add_trace(go.Bar(
                name=met,
                x=sub["Clase"],
                y=sub["Valor"],
                marker=dict(color=met_colors[i], opacity=0.85),
        ...
        fig_met.update_layout(
            **PLOTLY_TEMPLATE,
            ...
            height=360,
            margin=dict(l=0, r=0, t=10, b=0),
            yaxis=dict(range=[0, 1.15], **PLOTLY_TEMPLATE["yaxis"]),
    ...
    # ── Ablación ──
    st.markdown("<div class='sep'></div>", unsafe_allow_html=True)
    st.markdown(f"<div class='section-title'>Experimento de ablación</div>",
                unsafe_allow_html=True)
    st.markdown(f"""<div class='section-subtitle'>
        Impacto de eliminar las variables más informativas del modelo
    </div>""", unsafe_allow_html=True)
    ...
    fig_abl = go.Figure(go.Bar(
        x=abl_df["Accuracy"],
        y=abl_df["Configuración"],
        orientation="h",
        marker=dict(
            color=[CYAN, "#007A9E", "#005570", "#003355"],
    ...
    fig_abl.update_layout(
        **PLOTLY_TEMPLATE,
        ...
        xaxis=dict(range=[0.82, 0.94], tickformat=".0%", **PLOTLY_TEMPLATE["xaxis"]),
    )
...
# ══════════════════════════════════════════════
    ...
    reg_metrics = [
        ...
        ("13.856", "Individuos",        "ocupados c/ ingreso +"),
    ]
    for col, (val, lbl, dlt) in zip([r1, r2, r3, r4], reg_metrics):
        with col:
            st.markdown(f"""
            <div class="metric-card">
                <div class="metric-label">{lbl}</div>
    ...
    with cm1:
        st.markdown(f"<div class='section-title'>Comparación de modelos</div>",
                    unsafe_allow_html=True)
        st.markdown(f"""<div class='section-subtitle'>
            R² y RMSE en escala logarítmica para los tres modelos evaluados
        </div>""", unsafe_allow_html=True)
        ...
        for i, row in modelos_df.iterrows():
            fig_mod.add_trace(go.Bar(
                name=row["Modelo"].replace("\n", " "),
                x=[row["Modelo"].replace("\n", "<br>")],
                y=[row["R²_test"]],
                marker=dict(color=row["color"], opacity=0.85),
                text=f"{row['R²_test']:.4f}",
                textposition="outside",
                textfont=dict(color=WHITE, size=11),
                showlegend=False,
            ), row=1, col=1)
        ...
        for ax in ["xaxis", "xaxis2", "yaxis", "yaxis2"]:
            fig_mod.update_layout(**{ax: dict(gridcolor=BORDER, linecolor=BORDER,
                                              tickfont=dict(color=SLATE))})
        fig_mod["layout"]["yaxis"]["range"]  = [0, 0.65]
        fig_mod["layout"]["yaxis2"]["range"] = [0, 0.95]
        ...
        <div class="highlight-box">
            ...
            no aporta varianza reducida. Resultado metodológicamente esperado.
        </div>
        """, unsafe_allow_html=True)

    with cm2:
        st.markdown(f"<div class='section-title'>Efectos estimados sobre el ingreso</div>",
                    unsafe_allow_html=True)
        st.markdown(f"""<div class='section-subtitle'>
            Incremento porcentual en ingreso por unidad adicional de cada variable
            (coeficientes en escala logarítmica, Ridge α=500)
        </div>""", unsafe_allow_html=True)

        fig_coef = go.Figure(go.Bar(
            x=coefs_df["Efecto (% ingreso)"],
            y=coefs_df["Variable"],
            orientation="h",
            marker=dict(
                color=[GOLD if v >= 10 else CYAN if v >= 5 else TEAL
        ...
        fig_coef.update_layout(
            **PLOTLY_TEMPLATE,
            height=300,
            margin=dict(l=0, r=60, t=10, b=0),
            xaxis=dict(range=[0, 20], ticksuffix=" %", **PLOTLY_TEMPLATE["xaxis"]),
        )
    ...
    # ── Scatter residuos ──
    st.markdown(f"<div class='section-title'>Diagnóstico de residuos</div>",
                unsafe_allow_html=True)
    st.markdown(f"""<div class='section-subtitle'>
        Residuos vs. valores predichos en escala logarítmica.
        La dispersión homogénea confirma homocedasticidad razonable.
    </div>""", unsafe_allow_html=True)

    fig_res = go.Figure()
    fig_res.add_trace(go.Scatter(
        x=y_pred_log, y=residuos,
        mode="markers",
    ...
    fig_res.update_layout(
        **PLOTLY_TEMPLATE,
        height=280,
        margin=dict(l=0, r=0, t=10, b=0),
        xaxis_title="log(ingreso) predicho",
        yaxis_title="Residuo",
        showlegend=False,
    )
